  ...
  const termstmt = (data) => {
    ...
    if (data[0].type == "KEYWORD") {
      switch (data[0].value) {
        ...
        case "prompt":
          output[data[0].value] =
            strings[data[2].value.name.replace("TTRS:", "")];
        ...
        default:
          throw new Error(
            `termstmt ${JSON.stringify(data[0])} not implemented yet`
          );
      }
      ...
      data[0].type == "LBRACE" ||
      (data[0][0] && (data[0][0].type == "NL" || data[0][0].type === "_"))
    ) {
      output = data[1];
    }
    if (
      ...
      switch (data[0].value) {
        ...
        case "short":
          ...
          if (!output[data[0].value]) {
            throw new Error(
              "String not found " + JSON.stringify(data[2], null, 3)
            );
          }
          break;
        default:
          throw new Error(
            `Option ${JSON.stringify(data[0])} not implemented yet`
          );
      }
    } else if (data[0].type == "KEYWORD" && data[1] && !data[1].type) {
      data[1][data[0].value] = true;
  ...
  var grammar = {
    ...
    ParserRules: [
      ...
      { name: "main", symbols: ["main$ebnf$1"], postprocess: stmt },
      {
        name: "stmt",
        symbols: [
          lexer.has("TERMINAL") ? { type: "TERMINAL" } : TERMINAL,
      ...
      },
      {
        name: "stmt",
        symbols: [
          lexer.has("PLAYER") ? { type: "PLAYER" } : PLAYER,
          lexer.has("WHEN") ? { type: "WHEN" } : WHEN,
          "exp",
          "termblock",
        ],
      ...
      },
      {
        ...
        symbols: [
          ...
          lexer.has("IDENT") ? { type: "IDENT" } : IDENT,
          "termstmt",
        ],
        postprocess: termstmt,
      },
      ...
      },
      {
        ...
        symbols: [
          lexer.has("KEYWORD") ? { type: "KEYWORD" } : KEYWORD,
          lexer.has("COLON") ? { type: "COLON" } : COLON,
          lexer.has("OPT_STRING") ? { type: "OPT_STRING" } : OPT_STRING,
          "termstmt",
        ],
        postprocess: termstmt,
      },
      {
        ...
        symbols: [
          lexer.has("KEYWORD") ? { type: "KEYWORD" } : KEYWORD,
          lexer.has("COLON") ? { type: "COLON" } : COLON,
          lexer.has("INTER_STRING") ? { type: "INTER_STRING" } : INTER_STRING,
          "option_param",
        ],
        postprocess: termstmt,
      },
      {
        name: "termstmt",
        symbols: [
          lexer.has("OPTIONS") ? { type: "OPTIONS" } : OPTIONS,
          lexer.has("COLON") ? { type: "COLON" } : COLON,
          "option_stmt",
          "termstmt",
        ],
      ...
      {
        name: "termstmt",
        symbols: [lexer.has("IDENT") ? { type: "IDENT" } : IDENT, "termstmt"],
        postprocess: termstmt,
      },
      {
        name: "termstmt$subexpression$1",
        symbols: [lexer.has("_") ? { type: "_" } : _],
      },
      {
        name: "termstmt$subexpression$1",
      ...
      {
        name: "termstmt",
        symbols: ["termstmt$subexpression$1", "termstmt"],
        postprocess: termstmt,
      },
      {
        name: "termstmt",
      ...
      {
        name: "option_param$subexpression$1",
        symbols: [lexer.has("_") ? { type: "_" } : _],
      },
      {
        name: "option_param$subexpression$1",
      ...
      {
        name: "option_param",
        symbols: ["option_param$subexpression$1", "option_param"],
        postprocess: options,
      },
      {
        name: "option_param",
      ...
      {
        name: "exp$ebnf$1$subexpression$1",
        symbols: [lexer.has("OR") ? { type: "OR" } : OR, "term"],
      },
      {
        name: "exp$ebnf$1",
        symbols: ["exp$ebnf$1", "exp$ebnf$1$subexpression$1"],
        postprocess: function arrpush(d) {
          return d[0].concat([d[1]]);
        },
      },
      ...
      { name: "term$ebnf$1", symbols: [] },
      {
        ...
        symbols: [lexer.has("AND") ? { type: "AND" } : AND, "factor"],
      },
      {
        name: "term$ebnf$1",
      ...
      {
        name: "factor",
        symbols: [lexer.has("NOT") ? { type: "NOT" } : NOT, "factor"],
        postprocess: ident,
      },
      {
        name: "factor",
